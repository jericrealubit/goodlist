...
Generated by `npm run store:screenshots` into `docs/store/screenshots/` — real renders of the app at
1080x2160, 24-bit, no alpha. Upload all five under **Store listing → Phone screenshots**:

| File | What it shows |
|---|---|
| `01-my-tasks.png` | The personal list — notes, due dates, compose bar |
| `02-requested.png` | A task another member asked for, with their name on it |
| `03-group.png` | Invite code, members and their roles |
| `04-history.png` | Completed and cancelled tasks, with undo |
| `05-settings.png` | Profile and the theme picker |

The task edit form is not in the set — its due-date field renders as an HTML date input on web where
Android uses the native picker. Capture it on a device if you want it. See
`play-store-deployment.md` B6.
