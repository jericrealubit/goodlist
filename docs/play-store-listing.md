...
1080x2160, 24-bit, no alpha. Upload all four under **Store listing → Phone screenshots**:
...
The Group and Task-edit screens are not in the set because they render differently on web than on
Android; capture those on a device if you want them. See `play-store-deployment.md` B6.
