...
[![CI](https://github.com/jericrealubit/goodlist/actions/workflows/ci.yml/badge.svg)](https://github.com/jericrealubit/goodlist/actions/workflows/ci.yml)
...
- **Task alarms** — an opt-in phone alert at a task's exact due date and time, alongside it showing
  ...
  deleted.
- **Alarms ring until answered** — task alarms and medicine reminders use a loud alarm sound
  (`assets/sounds/alarm.wav`, generated by `scripts/generate-alarm-sound.mjs`) on Android alarm-volume
  channels, can't be swiped away, and keep ringing until the person taps Stop, Snooze or Done/Taken.
  With the app open, a full-screen alarm loops the sound and vibration (`expo-audio`); with it closed,
  follow-up notifications re-ring every 25 seconds for the first five minutes, then every minute,
  easing off to every half hour, for two hours (`FOLLOW_UP_OFFSETS_SEC` in `src/lib/alarms/ringing.ts`).
  On Android those rings stay one entry in the tray, not a pile: a local config plugin
  (`plugins/with-single-alarm-notification.js`) swaps in a subclass of expo-notifications' receiver
  that clears an alarm's earlier rings before showing the next. `USE_EXACT_ALARM` /
  `SCHEDULE_EXACT_ALARM` keep them on the minute. When an open task has an alarm but notifications
  are off, a banner at the top of the Calendar says so, with *Allow alarms* or *Open settings*. Saying *"remind me to X at 5pm"* turns the alarm on automatically; a plain *"add X"* never
...
- **Built-in guide** — an illustrated, 18-step walkthrough under Settings → Help, bundled with the
...
What could come next — search, push notifications for requests, alarm lead times, shared
repeating chores, OTA updates, crash reporting, iOS — ranked with the evidence for each, is in
...
plugins/          Local Expo config plugins. with-optional-microphone.js declares
                  ...
                  with-single-alarm-notification.js copies android/
                  AlarmNotificationsService.kt into the native project so an
                  alarm keeps one tray entry however many times it rings
.github/workflows/ci.yml  Typecheck, lint, unit tests, and a check that docs/ is
                  regenerated from src/content/ — on every push to main and PR
scripts/
...
| `npm test` | Run the unit tests (`src/**/*.test.ts`) with Node's built-in test runner — the pure modules under `src/lib/` |
| `npx tsc --noEmit` | Typecheck. Needs `expo-env.d.ts`, which `npm start` writes (CI writes it too) |
